...
select quantity,count(order_details_id) from order_details1 group by quantity;

select pizzas.size , count(order_details.order_details.id)
from pizzas join order_details1 
...
sum(order_details1.quantity)as quantity from
...
select round( avg (quentity),0) as order_pizzas_order_per_day from
(select orders.order_date, sum(order_details1.quantity) as quentity
from orders join order_details1 on
orders.order_id =order_details1.order_id 
...
sum(order_details1.quantity*pizzas.price) as revenue
from order_details1 join pizzas on
order_details1.pizza_id = pizzas.pizza_id join
orders on orders.order_id = order_details1.order_id
